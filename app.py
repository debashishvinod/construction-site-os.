...
# auto-refresh every 5 seconds, returns current run count
...
.intrusion-ring.secure{border-color:var(--green);color:var(--green);
...
<div class="dashboard">
  <div class="header">
    ...
    <div class="header-right">
      <div class="clock-block" id="st-clock">--:--:--</div>
      <div class="date-block"  id="st-date">-- --- ----</div>
      <div class="rc-wrap">
        <span class="rc-label">NEXT REFRESH</span>
        <div class="rc-bar"><div class="rc-fill" id="st-bar"></div></div>
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    ...
    <div class="footer-sync" id="st-sync">LAST SYNC: --:--:--</div>
  </div>
</div>
...
(function() {{
  ...
  function tick() {{
    ...
    var clk  = document.getElementById('st-clock');
    var dt   = document.getElementById('st-date');
    var sync = document.getElementById('st-sync');
    var bar  = document.getElementById('st-bar');

    if (clk)  clk.textContent  = t;
    if (dt)   dt.textContent   = dateStr;
    if (sync) sync.textContent = 'LAST SYNC: ' + t;

    // countdown bar: shrinks from 100% to 0% over 5 seconds
    if (bar) {{
      var elapsed  = (Date.now() - startTime) % INTERVAL;
      var remaining = 1 - (elapsed / INTERVAL);
