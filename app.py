...
# ── HELPERS: build card HTML as plain string concat (no nested f-strings) ──
def accent(is_alert):
...
workers_card = (
    ...
    +   f'<div class="card-value" style="color:#fff">{wc}</div>'
    + '</div>'
...
ppe_scls = "danger" if ppe_alert else ("warn" if ppe_warn else "")
ppe_card = (
    ...
    + '<div style="display:flex;align-items:baseline;gap:6px">'
    +   f'<div class="card-value" style="color:{ppe_c};text-shadow:0 0 30px {ppe_c}40">{ppe:.1f}</div>'
    +   '<span class="card-unit">%</span>'
    + '</div>'
    + bar_fill(pct(ppe, 100), ppe_c)
    + card_sub(ppe_sub, ppe_scls)
    + '</div>'
)
...
tmp_scls = "danger" if tmp_alert else ("warn" if tmp_warn else "")
tmp_card = (
    ...
    + '<div style="display:flex;align-items:baseline;gap:6px">'
    +   f'<div class="card-value" style="color:{tmp_c};text-shadow:0 0 30px {tmp_c}40">{tmp:.1f}</div>'
    +   '<span class="card-unit">°C</span>'
    + '</div>'
    + bar_fill(pct(tmp - 20, 30), tmp_c)
    + card_sub(tmp_sub, tmp_scls)
    + '</div>'
)

# Dust
dst_sub  = "CRITICAL — Respirators required" if dst_alert else ("Elevated — use precaution" if dst_warn else "Air quality nominal")
dst_scls = "danger" if dst_alert else ("warn" if dst_warn else "")
dst_card = (
    ...
    + '<div style="display:flex;align-items:baseline;gap:6px">'
    +   f'<div class="card-value" style="color:{dst_c};text-shadow:0 0 30px {dst_c}40">{dst:.0f}</div>'
    +   '<span class="card-unit">mg/m³</span>'
    + '</div>'
    + bar_fill(pct(dst, 80), dst_c)
    + card_sub(dst_sub, dst_scls)
    + '</div>'
)

# Crane
crn_sub  = "OVERLOAD — Halt operations immediately" if crn_alert else (f"High load — proceed with caution" if crn_warn else f"{crn:.0f}% capacity utilized")
crn_scls = "danger" if crn_alert else ("warn" if crn_warn else "")
crn_card = (
    ...
    + '<div style="display:flex;align-items:baseline;gap:6px">'
    +   f'<div class="card-value" style="color:{crn_c};text-shadow:0 0 30px {crn_c}40">{crn:.0f}</div>'
    +   '<span class="card-unit">%</span>'
    + '</div>'
    + bar_fill(min(pct(crn, 100), 100), crn_c)
    + card_sub(crn_sub, crn_scls)
    + '</div>'
)

# Zone Intrusion
zon_state = "breach" if zon else "secure"
zon_ring  = "ERR" if zon else "OK"
zon_sub   = "INTRUDER DETECTED — Evacuate restricted zone" if zon else "All zones secure"
zon_scls  = "danger" if zon else ""
zon_val   = "BREACH" if zon else "SECURE"
zon_card  = (
    ...
    + f'<div class="card-value" style="color:{zon_c};text-shadow:0 0 30px {zon_c}40;font-size:34px">{zon_val}</div>'
    + '<div class="intrusion-row">'
    +   f'<span class="intrusion-ring {zon_state}">{zon_ring}</span>'
    +   f'<div class="card-sub {zon_scls}" style="margin-top:0">{zon_sub}</div>'
    + '</div>'
    + '</div>'
)
...
# ── SENSOR PILLS ──────────────────────────────────────────────────────────────
sensors = ["PPE CAM", "DUST SENSOR", "CRANE LOAD", "THERMAL", "ZONE DETECT"]
sensor_pills = "".join(
...
.clock-block{font-family:var(--mono);font-size:26px;color:var(--amber);letter-spacing:0.08em;text-shadow:0 0 20px rgba(255,149,0,0.4);}
...
.rc-fill{height:100%;background:linear-gradient(90deg,var(--amber2),var(--amber));border-radius:1px;animation:refill 5s linear forwards;}
@keyframes refill{from{width:100%}to{width:0%}}
.divider{height:1px;background:linear-gradient(90deg,transparent,var(--amber),transparent);opacity:0.25;margin-bottom:22px;position:relative;}
.divider::after{content:'';position:absolute;top:-2px;left:50%;transform:translateX(-50%);width:6px;height:6px;background:var(--amber);border-radius:50%;box-shadow:0 0 12px var(--amber);}
.status-bar{display:flex;align-items:center;justify-content:space-between;background:var(--bg2);
  border:1px solid var(--border);border-radius:6px;padding:10px 18px;margin-bottom:16px;}
...
.metric-card{position:relative;background:var(--bg2);border:1px solid var(--border);border-radius:6px;padding:20px;overflow:hidden;transition:border-color 0.5s,box-shadow 0.5s;}
...
.metric-card::before,.metric-card::after{content:'';position:absolute;width:10px;height:10px;border-color:rgba(255,149,0,0.25);border-style:solid;}
...
.label-dot{width:4px;height:4px;border-radius:50%;box-shadow:0 0 6px currentColor;display:inline-block;animation:pulse-dot 2s ease-in-out infinite;}
...
.intrusion-ring{width:36px;height:36px;border-radius:50%;border:2px solid;display:inline-flex;align-items:center;justify-content:center;font-family:var(--mono);font-size:9px;font-weight:700;flex-shrink:0;}
...
.alerts-panel{border-radius:6px;border:1px solid rgba(255,45,45,0.25);background:#0c0606;overflow:hidden;margin-bottom:16px;}
...
.alert-row{display:flex;align-items:center;gap:12px;padding:11px 18px;border-bottom:1px solid rgba(255,255,255,0.03);}
...
.footer{display:flex;align-items:center;justify-content:space-between;padding-top:14px;border-top:1px solid rgba(255,255,255,0.05);}
